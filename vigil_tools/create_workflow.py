...
# Resolve repo root relative to this script. The `core` package these tools were
# written against lives in the cyber_soc_engine subtree, not at the STAI 2 root --
# pointing one level too high made `from core...` fail and silently dropped the
# soc_create_workflow tool from the registry.
REPO_ROOT = Path(__file__).resolve().parent.parent / "cyber_soc_engine"
